 ...
 */
static __global__
void _naive_fw_kernel(const int u, size_t pitch, const int nvertex, float* const graph, int* const pred) {
    ...
    if (y < nvertex && x < nvertex) {
        ...
        float newPath = graph[y * pitch + u] + graph[indexUX];
        float oldPath = graph[indexYX];
        if (oldPath > newPath) {
 ...
 */
static __global__
void _blocked_fw_dependent_ph(const int blockId, size_t pitch, const int nvertex, float* const graph, int* const pred) {
 ...
 */
static __global__
void _blocked_fw_partial_dependent_ph(const int blockId, size_t pitch, const int nvertex, float* const graph, int* const pred) {
 ...
 */
static __global__
void _blocked_fw_independent_ph(const int blockId, size_t pitch, const int nvertex, float* const graph, int* const pred) {
 ...
 */
static
size_t _cudaMoveMemoryToDevice(const std::unique_ptr<graphAPSPTopology>& dataHost, float** graphDevice, int** predDevice) {
 ...
 */
static
void _cudaMoveMemoryToHost(float* graphDevice, int* predDevice, const std::unique_ptr<graphAPSPTopology>& dataHost, size_t pitch) {
    size_t height = dataHost->nvertex;
    size_t width = height * sizeof(int);
